%% Questão 6
% A coorrente de uma antena do tipo loop circular (espira de raio a) pode ser modelada por
...
% Faça representações gráficas de A, B e E em um plano que contenha a espira e em um
% plano que contenha o eixo da espira,
    ...
    for i = 1:length(x)
        for j = 1:length(y)
            for k = 1:length(z)
                ...
                for u = 1:length(phil)
                    [rl(1), rl(2), rl(3)] = pol2cart(phil(u), a, 0);
                    
                    [Ir(1),Ir(2),Ir(3)]=pol2cart(phil(u)+pi/2,I0*sin(w*(t(p)- norm(r-rl)/c)),0);
                   
                    dA = (Ir) /norm(r-rl);
                    A(1,i,j,k,p) = A(1,i,j,k,p) + (Ir(1)/norm(r-rl))*a*dphi;
                    A(2,i,j,k,p) = A(2,i,j,k,p) + (Ir(2)/norm(r-rl))*a*dphi;
                    A(3,i,j,k,p) = A(3,i,j,k,p) + (Ir(3)/norm(r-rl))*a*dphi;
...
%%  GRÁFICO DO Potencial X TEMPO (YZ)
% figure(2);
% plot(t,squeeze(A(1,xmedio,ymedio,zmedio,:))');
% xlabel('tempo')
% ylabel('fluxo')
