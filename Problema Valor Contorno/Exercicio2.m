        ...
        for k = 1:length(z) % varre a coordenada z onde V será calculado
            ...
            for n=1:length(phil) % varre a coordenada  phil da carga
                for o=1:length(thetal) %varre a coordenada  thetal da carga
                    ...
                    [rl(1),rl(2),rl(3)]= sph2cart( thetal(o),phil(n),a);% vetor posição apontando para um elemento de volume dV.
                    [rll(1),rll(2),rll(3)]= sph2cart( thetal(o),phil(n),b);% vetor posição apontando para um elemento de volume dV.

                    dV1 = a^2*sin(thetal(o))*dphi*dtheta; 
                    dV2 = b^2*sin(thetal(o))*dphi*dtheta; 

                    if (sqrt((r-rl)*(r-rl)')>a*0.001)
                        V(i,j,k) = V(i,j,k)  + (1/(4*pi*e))*((ps1*dV1)/sqrt((r-rl)*(r-rl)'))';
                    end

                    if (sqrt((r-rll)*(r-rll)')>b*0.001)
