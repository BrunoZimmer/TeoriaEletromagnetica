...
ps = 6.5*10^-6; %densidade volumetrica de carga (em C/m^2)
...
%                     for m=1:length(rrl) %varre a coordenada  thetal da carga
                    ...
                    [rl(1),rl(2),rl(3)]= sph2cart( thetal(o),phil(n),rrl);% vetor posição apontando para um elemento de volume dV.

                    dV = rrl^2*sin(thetal(o))*dphi*dtheta; 

                    if (sqrt((r-rl)*(r-rl)')>0.01)
                        V(i,j,k) = V(i,j,k)  + (1/(4*pi*e0))*((ps*dV)/sqrt((r-rl)*(r-rl)'))';
                    end
